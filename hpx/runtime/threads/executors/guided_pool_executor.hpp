...
namespace hpx { namespace threads { namespace executors {
    ...
    struct pre_execution_async_domain_schedule
    {
        ...
        auto operator()(F&& f, Ts&&... ts) const
        {
            ...
            {
                p.apply(hpx::launch::sync, executor_.get_priority(),
            ...
            else
            {
                ...
                p.apply(hpx::launch::async, executor_.get_priority(),
                    executor_.get_stacksize(),
                    threads::thread_schedule_hint(
                        thread_schedule_hint_mode_numa, domain));
            }
    ...
    public:
                ...
                util::tuple<InnerFutures...>>::type>
        {
            ...
            {
                p.apply(hpx::launch::sync, pool_executor_.get_priority(),
            ...
            {
                p.apply(hpx::launch::async, pool_executor_.get_priority(),
                    pool_executor_.get_stacksize(),
                    threads::thread_schedule_hint(
                        thread_schedule_hint_mode_numa, domain));
            }
        ...
        async_execute(F&& f, Ts&&... ts)
        {
            ...
            else
            {
                ...
                lcos::local::futures_factory<result_type()> p(pool_exec_,
                    util::deferred_call(
                        std::forward<F>(f), std::forward<Ts>(ts)...));
                p.apply(hpx::launch::async, pool_exec_.get_priority(),
